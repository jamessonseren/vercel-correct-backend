...
<<<<<<< HEAD
        if(!props.card_name) throw new CustomError("Card name is required", 401)
        if(!props.card_type) throw new CustomError("Card type is required", 401)
        if(!props.correct_admin_id) throw new CustomError("Admin is required", 401)

        if(!(props.card_type in CardType)) throw new CustomError("Invalid card type", 401)
        if(typeof props.card_name !== 'string') throw new CustomError("Card name must be a String", 401)
        if(typeof props.correct_admin_id !== 'string') throw new CustomError("Card name must be a String", 401)
=======
